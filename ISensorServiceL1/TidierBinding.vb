...
Namespace Nist.Bcl.Wsbd
    ...
    Public Class TidierInspector
        Implements IDispatchMessageInspector, IEndpointBehavior


        Public Function AfterReceiveRequest(ByRef request As Message, ByVal channel As IClientChannel,
                                            ByVal instanceContext As InstanceContext) As Object _
        ...
        Public Sub BeforeSendReply(ByRef reply As Message, ByVal correlationState As Object) Implements IDispatchMessageInspector.BeforeSendReply
            '
            ' Make sure we only transform XML messages. In most cases, only XML will be used,
            ' but in case there is a configuration that sends replies in a format other than XML,
            ' having this check may prevent throwing exceptions that might be hard to troubleshoot.
            '
            If Tidier.GetMessageContentFormat(reply) = WebContentFormat.Xml Then
                Tidier.TransformMessage(reply)
            End If
